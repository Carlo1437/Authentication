...
// We only want to replace inside class="..."
content = content.replace(/class="([^"]+)"/g, (match, classes) => {
  ...
  return `class="${newClasses} transition-colors duration-300"`;
});

// Since we appended the dark mode btn manually already, we should ensure we don't mess it up
// Navbar button injection
const loginTarget = /<NuxtLink\s+to="\/login"/;
if (loginTarget.test(content) && !content.includes('toggleTheme')) {
  const buttonCode = `<button @click="toggleTheme" class="p-2 mr-2 rounded-full text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800 transition-colors" aria-label="Toggle Dark Mode">
